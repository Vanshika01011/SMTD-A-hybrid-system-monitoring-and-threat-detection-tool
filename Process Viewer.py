...
class LiveGraphFrame(ttk.Frame):
    def __init__(self, parent):
        ...
        self.ani = FuncAnimation(
            self.fig, self.update_graph, interval=1000, blit=True, cache_frame_data=False
        )
...
class ProcessViewerFrame(ttk.Frame):
    def __init__(self, parent):
        ...
        search_entry.bind("<KeyRelease>", lambda e: self.update_processes())
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_percent']):
            try:
                ...
                if search_term in name.lower():
                    self.tree.insert("", "end", values=(pid, name, round(cpu, 1), round(mem, 1)))
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                continue

        self.after(3000, self.update_processes)  # Update every 3 seconds
